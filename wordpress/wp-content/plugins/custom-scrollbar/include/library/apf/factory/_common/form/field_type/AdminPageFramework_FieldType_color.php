...
abstract class CustomScrollbar_AdminPageFramework_FieldType_Base extends CustomScrollbar_AdminPageFramework_Form_Utility {
    ...
    protected function getFieldOutput(array $aFieldset) {
        ...
        $_oFieldset = new CustomScrollbar_AdminPageFramework_Form_View___Fieldset($aFieldset, $_oCallerForm->aSavedData, $_oCallerForm->getFieldErrors(), $_oCallerForm->aFieldTypeDefinitions, $_oCallerForm->oMsg, $_oCallerForm->aCallbacks);
    ...
    protected function enqueueMediaUploader() {
        ...
        if (function_exists('wp_enqueue_media')) {
            new CustomScrollbar_AdminPageFramework_Form_View___Script_MediaUploader($this->oMsg);
        } else {
...
abstract class CustomScrollbar_AdminPageFramework_FieldType extends CustomScrollbar_AdminPageFramework_FieldType_Base {
    public function _replyToFieldLoader() {
        $this->setUp();
    }
    public function _replyToGetScripts() {
...
class CustomScrollbar_AdminPageFramework_FieldType_color extends CustomScrollbar_AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".repeatable .colorpicker {display: inline;}.custom-scrollbar-field-color .wp-picker-container {vertical-align: middle;}.custom-scrollbar-field-color .ui-widget-content {border: none;background: none;color: transparent;}.custom-scrollbar-field-color .ui-slider-vertical {width: inherit;height: auto;margin-top: -11px;}.custom-scrollbar-field-color .custom-scrollbar-repeatable-field-buttons {margin-top: 0;}.custom-scrollbar-field-color .wp-color-result {margin: 3px;}";
    }
    protected function getScripts() {
        $_aJSArray = json_encode($this->aFieldTypeSlugs);
        $_sDoubleQuote = '\"';
        return <<<JAVASCRIPTS
registerCustomScrollbar_AdminPageFrameworkColorPickerField = function( osTragetInput, aOptions ) {
    ...
    var _aDefaults = {
        ...
        change: function( event, ui ){
            jQuery( this ).trigger( 
                'custom-scrollbar_field_type_color_changed',
        ...
        clear: function( event, ui ) {
            jQuery( this ).trigger(
                'custom-scrollbar_field_type_color_cleared',
    ...
    jQuery().registerCustomScrollbar_AdminPageFrameworkCallbacks( {     
        added_repeatable_field: function( oClonedField, sFieldType, sFieldTagID, sCallType ) {
            ...
            oClonedField.find( 'input.input_color' ).each( function( iIterationIndex ) {
                ...
                registerCustomScrollbar_AdminPageFrameworkColorPickerField( _oNewColorInput );                
...
JAVASCRIPTS;
    ...
    protected function getField($aField) {
        ...
        return $aField['before_label'] . "<div class='custom-scrollbar-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'custom-scrollbar-input-label-string') . ">" . $aField['label'] . "</span>" : "") . "<input " . $this->getAttributes($aField['attributes']) . " />" . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "<div class='colorpicker' id='color_{$aField['input_id']}'></div>" . $this->_getColorPickerEnablerScript("{$aField['input_id']}") . "</div>" . $aField['after_label'];
    }
...
jQuery( document ).ready( function(){
    registerCustomScrollbar_AdminPageFrameworkColorPickerField( '{$sInputID}' );
});            
JAVASCRIPTS;
