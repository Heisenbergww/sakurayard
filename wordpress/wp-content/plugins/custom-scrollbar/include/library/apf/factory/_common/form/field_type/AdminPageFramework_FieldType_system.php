...
class CustomScrollbar_AdminPageFramework_FieldType_system extends CustomScrollbar_AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".custom-scrollbar-field-system {width: 100%;}.custom-scrollbar-field-system .custom-scrollbar-input-label-container {width: 100%;}.custom-scrollbar-field-system textarea {background-color: #f9f9f9; width: 97%; outline: 0; font-family: Consolas, Monaco, monospace;white-space: pre;word-wrap: normal;overflow-x: scroll;}";
    }
    protected function getField($aField) {
        $_aInputAttributes = $aField['attributes'];
        $_aInputAttributes['class'].= ' system';
        unset($_aInputAttributes['value']);
        return $aField['before_label'] . "<div class='custom-scrollbar-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'custom-scrollbar-input-label-string') . ">" . $aField['label'] . "</span>" : "") . "<textarea " . $this->getAttributes($_aInputAttributes) . " >" . esc_textarea($this->_getSystemInfomation($aField['value'], $aField['data'], $aField['print_type'])) . "</textarea>" . $aField['after_input'] . "</label>" . "</div>" . $aField['after_label'];
    }
    ...
    private function _getClientInfo($bGenerateInfo = true) {
        ...
        return empty($_aBrowser) ? __('No browser information found.', 'custom-scrollbar') : $_aBrowser;
    }
    ...
    private function _getSiteInfo() {
        global $wpdb;
        return array(__('Version', 'custom-scrollbar') => $GLOBALS['wp_version'], __('Language', 'custom-scrollbar') => $this->getSiteLanguage(), __('Memory Limit', 'custom-scrollbar') => $this->getReadableBytes($this->getNumberOfReadableSize(WP_MEMORY_LIMIT)), __('Multi-site', 'custom-scrollbar') => $this->getAOrB(is_multisite(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Permalink Structure', 'custom-scrollbar') => get_option('permalink_structure'), __('Active Theme', 'custom-scrollbar') => $this->_getActiveThemeName(), __('Registered Post Statuses', 'custom-scrollbar') => implode(', ', get_post_stati()), 'WP_DEBUG' => $this->getAOrB($this->isDebugMode(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_LOG' => $this->getAOrB($this->isDebugLogEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_DISPLAY' => $this->getAOrB($this->isDebugDisplayEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Table Prefix', 'custom-scrollbar') => $wpdb->prefix, __('Table Prefix Length', 'custom-scrollbar') => strlen($wpdb->prefix), __('Table Prefix Status', 'custom-scrollbar') => $this->getAOrB(strlen($wpdb->prefix) > 16, $this->oMsg->get('too_long'), $this->oMsg->get('acceptable')), 'wp_remote_post()' => $this->_getWPRemotePostStatus(), 'wp_remote_get()' => $this->_getWPRemoteGetStatus(), __('Multibite String Extension', 'custom-scrollbar') => $this->getAOrB(function_exists('mb_detect_encoding'), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('WP_CONTENT_DIR Writeable', 'custom-scrollbar') => $this->getAOrB(is_writable(WP_CONTENT_DIR), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Active Plugins', 'custom-scrollbar') => PHP_EOL . $this->_getActivePlugins(), __('Network Active Plugins', 'custom-scrollbar') => PHP_EOL . $this->_getNetworkActivePlugins(), __('Constants', 'custom-scrollbar') => $this->_getDefinedConstants('user'),);
    }
    ...
    private function _getPHPInfo($bGenerateInfo = true) {
        ...
        self::$_aPHPInfo = array(__('Version', 'custom-scrollbar') => phpversion(), __('Safe Mode', 'custom-scrollbar') => $this->getAOrB(@ini_get('safe_mode'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Memory Limit', 'custom-scrollbar') => @ini_get('memory_limit'), __('Upload Max Size', 'custom-scrollbar') => @ini_get('upload_max_filesize'), __('Post Max Size', 'custom-scrollbar') => @ini_get('post_max_size'), __('Upload Max File Size', 'custom-scrollbar') => @ini_get('upload_max_filesize'), __('Max Execution Time', 'custom-scrollbar') => @ini_get('max_execution_time'), __('Max Input Vars', 'custom-scrollbar') => @ini_get('max_input_vars'), __('Argument Separator', 'custom-scrollbar') => @ini_get('arg_separator.output'), __('Allow URL File Open', 'custom-scrollbar') => $this->getAOrB(@ini_get('allow_url_fopen'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Display Errors', 'custom-scrollbar') => $this->getAOrB(@ini_get('display_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Log Errors', 'custom-scrollbar') => $this->getAOrB(@ini_get('log_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Error log location', 'custom-scrollbar') => @ini_get('error_log'), __('Error Reporting Level', 'custom-scrollbar') => $_oErrorReporting->getErrorLevel(), __('FSOCKOPEN', 'custom-scrollbar') => $this->getAOrB(function_exists('fsockopen'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('cURL', 'custom-scrollbar') => $this->getAOrB(function_exists('curl_init'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SOAP', 'custom-scrollbar') => $this->getAOrB(class_exists('SoapClient'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SUHOSIN', 'custom-scrollbar') => $this->getAOrB(extension_loaded('suhosin'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), 'ini_set()' => $this->getAOrB(function_exists('ini_set'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')),) + $this->getPHPInfo() + array(__('Constants', 'custom-scrollbar') => $this->_getDefinedConstants(null, 'user'));
        return self::$_aPHPInfo;
    }
    private function _getWebServerInfo($bGenerateInfo = true) {
        return $bGenerateInfo ? array(__('Web Server', 'custom-scrollbar') => $_SERVER['SERVER_SOFTWARE'], 'SSL' => $this->getAOrB(is_ssl(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Session', 'custom-scrollbar') => $this->getAOrB(isset($_SESSION), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Session Name', 'custom-scrollbar') => esc_html(@ini_get('session.name')), __('Session Cookie Path', 'custom-scrollbar') => esc_html(@ini_get('session.cookie_path')), __('Session Save Path', 'custom-scrollbar') => esc_html(@ini_get('session.save_path')), __('Session Use Cookies', 'custom-scrollbar') => $this->getAOrB(@ini_get('session.use_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Session Use Only Cookies', 'custom-scrollbar') => $this->getAOrB(@ini_get('session.use_only_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')),) + $_SERVER : '';
    }
}
